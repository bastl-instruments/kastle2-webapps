...
function useFirmwareGeneration() {
    ...
    const firmware = alchemistContext.firmware || firmwares.find((x) => x.default);

    const generateUf2Firmware = async () => {

        // Convert the data and make a UF2 file from them
        const binArray = generateBinaryData(alchemistContext);
    ...
    const generateBinarySamples = () => {
        const firmwareArray = generateBinaryData(alchemistContext);
