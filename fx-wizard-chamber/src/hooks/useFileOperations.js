...
export default function useFileOperations() {
    ...
    const loadFile = async (file, unsupportedMessage = null) => {
        ...
        try {
            ...
            if (isUf2) {
                ...
                if (firmware) {
                    fxWizardContext.setRhythms(firmware.rhythms || []);
                    fxWizardContext.setSequencerLength(firmware.sequencerLength || fxWizardSequencerLength);
                } else {
                    throw new Error('Failed to parse firmware');
                }
            } else if (isDraft) {
                await loadWorkFile(file, fxWizardContext);
            }
        } catch (error) {
    ...
    const saveCurrentWorkFile = async () => {
        try {
            ...
            await saveWorkFile(fxWizardContext);
        } catch (error) {
